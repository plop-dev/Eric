...
def search_song(query: str):
    ...
    result_raw = sp.search(
        f"{track} {artist}", limit=1, offset=0, type="track", market="GB"
    )
    print(result_raw)
    result = result_raw["tracks"]["items"][0][
        list(result_raw["tracks"]["items"][0].keys())[0]
    ]["uri"]
    print(result)
    return result
...
class SearchSong(Command):
    def primary(self):
        if "play the song" in self.user_query:
            ...
            return EricUtils.UserQueryError(
                "Command 'play' needed for searching songs."
            )
...
class PlayMainPlaylist(Command):
    def primary(self):
        sp.start_playback(
            SPOTIFY_DEVICE_ID,
            "https://open.spotify.com/playlist/1QjJZaw3mysDIPaRlXIfH7",
        )
        return None
